...
function renderMap(data) {
  ...
  data['scheme'].forEach((item, i) => {
    const
      letters = item === 4 ? [].concat('', data.letters4, '') : ((item === 6) ? data.letters6 : []),
      row = elRender('div', {class: 'row seating-row text-center'}, [
        elRender('div', {class: 'col-xs-1 row-number'}, [
          elRender('h2', null, `${i + 1}`)
        ])
      ]),
      colLeft = elRender('div', {class: 'col-xs-5'}),
      colRight = elRender('div', {class: 'col-xs-5'});
...
acSelect.addEventListener('change', event => getPlaneData(event.target.value));

document.addEventListener('DOMContetLoaded', getPlaneData(acSelect.value));
